...

{
  ...
  while (time < t_final) //for ( it = 1; it <= nt; it++ )
  {
	  ...
	  if (rank_left >= 0){
		  ...
		  MPI_Irecv(buf_left_recv, (3 * ny_loc), MPI_DOUBLE, rank_left,
			  101, MPI_COMM_WORLD, &request_recv_left);


		  //set up sending to the left
		  for (int k = 0; k < ny_loc; k++)
		  {
			  j = 1;
			  id = ID_2D(k + 1, j, nx_loc);
			  buf_left_send[k * 3 + 0] = h[id];
			  buf_left_send[k * 3 + 1] = uh[id];
			  buf_left_send[k * 3 + 2] = vh[id];
		  }
		  MPI_Isend(buf_left_send, (3 * ny_loc), MPI_DOUBLE, rank_left,
	  ...
	  if (rank_right >= 0){
		  ...
		  MPI_Irecv(buf_right_recv, (3 * ny_loc), MPI_DOUBLE, rank_right,
		  ...
		  for (int k = 0; k < ny_loc; k++)
		  {
			  j = nx_loc;
			  id = ID_2D(k + 1, j, nx_loc);
			  buf_right_send[k * 3 + 0] = h[id];
			  buf_right_send[k * 3 + 1] = uh[id];
			  buf_right_send[k * 3 + 2] = vh[id];
		  }
	  ...
	  if (rank_down >= 0){
		  ...
		  MPI_Irecv(buf_down_recv, (3 * nx_loc), MPI_DOUBLE, rank_down,
		  ...
		  {
			  i = 1;
			  id = ID_2D(i, k + 1, nx_loc);
			  buf_down_send[k * 3 + 0] = h[id];
			  buf_down_send[k * 3 + 1] = uh[id];
			  buf_down_send[k * 3 + 2] = vh[id];
		  }
		  MPI_Isend(buf_down_send, (3 * nx_loc), MPI_DOUBLE, rank_down,
	  ...
	  if (rank_up >= 0){
		  ...
		  MPI_Irecv(buf_up_recv, (3 * nx_loc), MPI_DOUBLE, rank_up,
		  ...
		  for (int k = 0; k < nx_loc; k++)
		  {
			  i = ny_loc;
			  id = ID_2D(i, k + 1, nx_loc);
			  buf_up_send[k * 3 + 0] = h[id];
			  buf_up_send[k * 3 + 1] = uh[id];
			  buf_up_send[k * 3 + 2] = vh[id];
		  }
		  MPI_Isend(buf_up_send, (3 * ny_loc), MPI_DOUBLE, rank_up,
	  ...
	  if (rank_up >= 0) {
		  ...
		  for (j = 0; j < nx_loc; j++) {
			  ...
			  h[id] = buf_up_recv[j * 3 + 0];
			  uh[id] = buf_up_recv[j * 3 + 1];
			  vh[id] = buf_up_recv[j * 3 + 2];
		  }
	  }
	  else { // apply physical BC
		  for (j = 0; j < nx_loc; j++) {
	  ...
	  if (rank_down >= 0) {
		  ...
		  for (j = 0; j < nx_loc; j++) {
			  i = 0;
			  id = ID_2D(i, j + 1, nx_loc);
			  h[id] = buf_down_recv[j * 3 + 0];
			  uh[id] = buf_down_recv[j * 3 + 1];
			  vh[id] = buf_down_recv[j * 3 + 2];
		  }
	  }
	  else { // apply physical BC
		  for (j = 0; j < nx_loc; j++) {
...
void initial_conditions(int nx, int ny, double dx, double dy,
	double x_length, double x[], double y[], double h[],
	double uh[], double vh[], int irank, int q)
