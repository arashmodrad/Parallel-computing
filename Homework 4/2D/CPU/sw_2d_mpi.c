...
//#define ID_Loc(i, j, nx_loc) (i * nx_loc + j)

int main(int argc, char *argv[]);
void initial_conditions(int q, int nx, int ny, double dx, double dy,
                        double x_length, double x[], double y[], double h[],
                        double uh[], double vh[], int irank);
void compute_flux_2d_point(double fh, double fuh, double fvh, double gh,
                           double guh, double gvh, double h, double uh,
                           double vh, double g);

// utilities
void getArgs_mpi(int *nx, double *dt, double *x_length, double *t_final,
                 int argc, char *argv[], int irank, MPI_Comm comm);

void write_results(char *output_filename, int nx, int nx_loc, int ny,
                   int ny_loc, double x[], double y[], double h[], double uh[],
                   double vh[], int irank, int nproc);
...
/*
  ...
  Parameters:
    ...
    Output, real UH[NX], the mass velocity (discharge) for all space points at
  time t_final.
*/

{
  ...
  double *gh;
  double *hm;
  int i, j, id, id1, id_left, id_right, id_bottom, id_top;
  ...
  initial_conditions(q, nx_loc, ny_loc, dx, dy, x_length, x, y, h, uh, vh,
                     irank);

  // Write initial condition to a file
  write_results("sw2d_init.dat", nx, nx_loc, nx, ny_loc, x, y, h, uh, vh, irank,
                nproc);

  double lambda_x = 0.5 * dt / dx;
  double lambda_y = 0.5 * dt / dy;

  double buf_left_recv[3*ny_loc], buf_right_recv[3*ny_loc], buf_top_recv[3*nx_loc],
      buf_bottom_recv[3*nx_loc];
  double buf_left_send[3*ny_loc], buf_right_send[3*ny_loc], buf_top_send[3*nx_loc],
      buf_bottom_send[3*nx_loc];

  MPI_Status status;

  MPI_Request request_recv_left, request_recv_right, request_recv_top,
      request_recv_bottom;
  MPI_Request request_send_left, request_send_right, request_send_top,
      request_send_bottom;

  // SETUP UP CARTESIAN COMMUNICATOR - ONLY LOOKS FOR NEIGHBORs EXISTENCE
  MPI_Comm Comm_cart;
  int ndims = 2; // number of dimensions of cartesian grid
  int rank_left, rank_right, rank_top, rank_bottom;

  int wrap_around[2];
  wrap_around[0] = 0; // no periodicity top-bottom
  wrap_around[1] = 0; // no periodicity left-right
  ...
  MPI_Cart_shift(Comm_cart, 0, 1, &rank_bottom, &rank_top);//reversed to correct for ranks running bottom-left to top-right
  ...
  while (time < t_final) // for ( it = 1; it <= nt; it++ )
  {
    ...
    // if there is a Rank to the left
    if (rank_left >= 0) {
      // receive column from left
      MPI_Irecv(buf_left_recv, 3*ny_loc, MPI_DOUBLE, rank_left, 101, MPI_COMM_WORLD,
                &request_recv_left);

      // send column on right to the left
      for (i = 0; i < ny_loc; i++) {
        j = 1;
        id = ID_2D(i+1, j, nx_loc);
        buf_left_send[i*3+0] = h[id];
        buf_left_send[i*3+1] = uh[id];
        buf_left_send[i*3+2] = vh[id];
      }
      MPI_Isend(buf_left_send, 3*ny_loc, MPI_DOUBLE, rank_left, 101, MPI_COMM_WORLD,
                &request_send_left);
    }

    // if there is a Rank to the right
    if (rank_right >= 0) {
      // receive column from right
      MPI_Irecv(buf_right_recv, 3*ny_loc, MPI_DOUBLE, rank_right, 101, MPI_COMM_WORLD,
                &request_recv_right);

      // send column on the left to the right
      for (i = 0; i < ny_loc; i++) {
        j = nx_loc;
        id = ID_2D(i+1, j, nx_loc);
        buf_right_send[i*3+0] = h[id];
        buf_right_send[i*3+1] = uh[id];
        buf_right_send[i*3+2] = vh[id];
      }
      MPI_Isend(buf_right_send, 3*ny_loc, MPI_DOUBLE, rank_right, 101,
                MPI_COMM_WORLD, &request_send_right);
    }
    // if there is a Rank above
    if (rank_top >= 0) {
      // receive from top
      MPI_Irecv(buf_top_recv, 3*nx_loc, MPI_DOUBLE, rank_top, 101, MPI_COMM_WORLD,
                &request_recv_top);

      // send column of h, uh, uv values to the top
      for (j = 0; j < nx_loc; j++) {
        i = ny_loc;
        id = ID_2D(i, j+1, nx_loc);
        buf_top_send[j*3+0] = h[id];
        buf_top_send[j*3+1] = uh[id];
        buf_top_send[j*3+2] = vh[id];
      }
      MPI_Isend(buf_top_send, 3*nx_loc, MPI_DOUBLE, rank_top, 101, MPI_COMM_WORLD,
                &request_send_top);
    }

    // if there is a Rank below
    if (rank_bottom >= 0) {
      // receive from bottom
      MPI_Irecv(buf_bottom_recv, 3*nx_loc, MPI_DOUBLE, rank_bottom, 101,
                MPI_COMM_WORLD, &request_recv_bottom);

      // send column of h, uh, uv values to the bottom
      for (j = 0; j < nx_loc; j++) {
        i = 1;
        id = ID_2D(i, j+1, nx_loc);
        buf_bottom_send[j*3+0] = h[id];
        buf_bottom_send[j*3+1] = uh[id];
        buf_bottom_send[j*3+2] = vh[id];
        }
      MPI_Isend(buf_bottom_send, 3*nx_loc, MPI_DOUBLE, rank_bottom, 101,
                MPI_COMM_WORLD, &request_send_bottom);
    }

//******************** END COMMUNICATION SETUP  *******************
    ...
    for (i = 1; i < ny_loc + 1; i++)
      for (j = 1; j < nx_loc + 1; j++) {
        ...
        fuh[id] =
            uh[id] * uh[id] / h[id] +
            0.5 * g * h[id] *
                h[id]; // flux for the momentum equation: u^2*h + 0.5*g*h^2
        fvh[id] =
            uh[id] * vh[id] / h[id]; // flux for the momentum equation: u*v**h
        ...
        gvh[id] =
            vh[id] * vh[id] / h[id] +
            0.5 * g * h[id] *
                h[id]; // flux for the momentum equation: v^2*h + 0.5*g*h^2
      }
    ...
    for (i = 2; i < ny_loc; i++)
      for (j = 2; j < nx_loc; j++) {
        ...
        uhm[id] =
            0.25 * (uh[id_left] + uh[id_right] + uh[id_bottom] + uh[id_top]) -
            lambda_x * (fuh[id_right] - fuh[id_left]) -
            lambda_y * (guh[id_top] - guh[id_bottom]);
        vhm[id] =
            0.25 * (vh[id_left] + vh[id_right] + vh[id_bottom] + vh[id_top]) -
    ...
    if (rank_left >= 0) {
      //printf("Buffer Left - Rank:%i, Left:%i\n", irank, rank_left);
      MPI_Wait(&request_recv_left, &status);

      //Unwind receive buffer
      for (i = 0; i < ny_loc; i++) {
        j = 0;
        id = ID_2D(i+1, j, nx_loc);
        h[id] = buf_left_recv[i*3+0];
        uh[id] = buf_left_recv[i*3+1];
        vh[id] = buf_left_recv[i*3+2];
      }
    } else { // apply physical BC
      for (i = 0; i < ny_loc; i++) {
        j = 0;
        id = ID_2D(i+1, j, nx_loc);
        id1 = ID_2D(i+1, j+1, nx_loc);//added +1 to j
        h[id] = h[id1];
        uh[id] = -uh[id1];
        vh[id] = vh[id1];
      }
    ...
    if (rank_right >= 0) {
      ...
      //Unwind receive buffer
      for (i = 0; i < ny_loc; i++) {
        j = nx_loc + 1; //change -1 to +1
        id = ID_2D(i+1, j, nx_loc);
        h[id] = buf_right_recv[i*3+0];
        uh[id] = buf_right_recv[i*3+1];
        vh[id] = buf_right_recv[i*3+2];
      }
    } else { // apply physical BC
      for (i = 0; i < ny_loc; i++) {
        j = nx_loc+1;//change -1 to +1
    ...
    if (rank_top >= 0) {
      MPI_Wait(&request_recv_top, &status);

      //Unwind receive buffer
      for (j = 0; j < nx_loc; j++) {
        i = ny_loc + 1; // added +1
        id = ID_2D(i, j+1, nx_loc);
        h[id] = buf_top_recv[j*3+0];
        uh[id] = buf_top_recv[j*3+1];
        vh[id] = buf_top_recv[j*3+2];
      }
    } else { // apply physical BC
      for (j = 0; j < nx_loc; j++) {
        i = ny_loc + 1;// added +1
        id = ID_2D(i, j+1, nx_loc);
        id1 = ID_2D(i-1, j+1, nx_loc);//add -1 to i
        h[id] = h[id1];
        uh[id] = uh[id1];
        vh[id] = -vh[id1];
      }
    ...
    if (rank_bottom >= 0) {
      MPI_Wait(&request_recv_bottom, &status);

      //Unwind receive buffer
      for (j = 0; j < nx_loc; j++) {
        i = 0;
        id = ID_2D(i, j+1, nx_loc);
        h[id] = buf_bottom_recv[j*3+0];
        uh[id] = buf_bottom_recv[j*3+1];
        vh[id] = buf_bottom_recv[j*3+2];
        }
    } else { // apply physical BC
      for (j = 0; j < nx_loc; j++) {
        i = 0;
        id = ID_2D(i, j+1, nx_loc);
        id1 = ID_2D(i+1, j+1, nx_loc);//added 1 to i
        h[id] = h[id1];
        uh[id] = uh[id1];
        vh[id] = -vh[id1];
        }
  ...
  // Capture elapsed time, print results
   sw2dtime = MPI_Wtime() - sw2dtime;
   if (irank == 0){
     printf("%i\t%i\t%e\n", nproc, nx, sw2dtime);
   }
...
void compute_flux_2d_point(double fh, double fuh, double fvh, double gh,
  ...
  gvh = vh * vh / h +
        0.5 * g * h * h; // flux for the momentum equation: v^2*h + 0.5*g*h^2
}
...

{
  ...
  MPI_Gatherv(uh_send, nx_loc * ny_loc, MPI_DOUBLE, uh_print, recvcount, displs,
              MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Gatherv(vh_send, nx_loc * ny_loc, MPI_DOUBLE, vh_print, recvcount, displs,
              MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Gatherv(h_send, nx_loc * ny_loc, MPI_DOUBLE, h_print, recvcount, displs,
              MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Gatherv(x_send, nx_loc * ny_loc, MPI_DOUBLE, x_print, recvcount, displs,
              MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Gatherv(y_send, nx_loc * ny_loc, MPI_DOUBLE, y_print, recvcount, displs,
              MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    for (i = 0; i < ny; i++)
      for (j = 0; j < nx; j++) {
        fprintf(output, "  %24.16g\t%24.16g\t%24.16g\t %24.16g\t %24.16g\n",
                x_print[i * nx + j], y_print[i * nx + j], h_print[i * nx + j],
                uh_print[i * nx + j], vh_print[i * nx + j]);
      }
